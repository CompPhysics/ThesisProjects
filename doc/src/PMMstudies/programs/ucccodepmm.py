"""
Pairing Hamiltonian + UCC-like pair-excitation ansatz + Trotterization +
MLP and PMM regressors (no Qiskit).

Features:
  - Pairing Hamiltonian with L spatial levels (2L spin orbitals/qubits).
  - Exact many-body representation in Fock space (dimension 2^(2L)).
  - UCCSD-like generator restricted to pair excitations from occupied to
    unoccupied levels:
        T = sum_{p in occ, q in virt} theta * P_q^† P_p
    with P_p^† = c_{p↑}^† c_{p↓}^†.
    The anti-Hermitian generator is:
        K = i (T - T^†)  (Hermitian operator A = K)
    and the unitary is approximated via Trotter:
        U(θ) = exp(-i θ Σ_j A_j)
        ≈ [ ∏_j exp(-i (θ / n_steps) A_j) ]^{n_steps}.

  - We build A_j from pair excitation operators and simulate the Trotterized
    UCC state in the full Fock space.

  - Pairing Hamiltonian:
        H = Σ_p eps_p (n_{p↑} + n_{p↓}) - g Σ_{p,q} P_p^† P_q

  - HF reference: first N_pairs levels doubly occupied.

  - Dataset:
        Features x = [dt, g, theta]
        dt = theta / n_steps
        Target y = E_trotter = <ψ(θ, n_steps)|H|ψ(θ, n_steps)>

  - Exact ground-state energy from diagonalization of H.

  - Regressors:
        (1) MLP: R^3 -> R
        (2) PMM: parametric Hermitian primary + secondary matrix, using
            eigenvectors to form scalar outputs (energy).

  - Extrapolation:
        For chosen (g*, θ*), generate energies vs dt with Trotter,
        and predictions from MLP and PMM, and compare to exact energy.
