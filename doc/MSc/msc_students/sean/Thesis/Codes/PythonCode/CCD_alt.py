...
NB 	= 5							#number of closed-shells (n^2=0, n^2=1, n^2=2, etc... For NB=2 can at max have N=14)
...
def temp1(inMat):
	...
	for row in range(0,dim):
		for col in range(0,dim):
			vMat[row,col] = system.assym(inMat[row,0],inMat[row,1],inMat[col,0],inMat[col,1])
	return vMat
    
def temp2(inMat1,intMat2):
	dim = np.shape(inMat)[0]
	vMat = np.zeros((dim,dim))
	for row in range(0,dim):
		for col in range(0,dim):
...
def makeIntMatrix():	#just fyi, this function took me half a day, I'm not fluent in numpy functions
	...
	distinct_k_u = np.shape(np.unique(blockArrays[:,0]))[0]
	lastIndices = np.zeros(distinct_k_u)	#array with the last index for each k_u, which seperates the blocks
	index = 0
	#BEWARE: for-loop notation might be wrong here
	for val in np.unique(blockArrays[:,0]):					#val now takes all different values of k_u
		lastIndices[index] = np.where(blockArrays[:,0] == val)[0][-1]		#you get back a tuple where the first element is an array with indices for blockArrays==val)
		index += 1
	
	blockArraysNew = []	#i think list is best, due to the varying size of the elements
	prev_i = 0
	for i in lastIndices:
		blockArraysNew.append(np.zeros((i+1-prev_i,2)))
		blockArraysNew[-1] = blockArrays[prev_i:i,1:]
		prev_i = i
	
	intMat_pppp = []
	for i in range(0,distinct_k_u):	#this loop ought to disappear once assym is vectorized
		intMat_pppp.append(temp1(blockArraysNew[i]))
