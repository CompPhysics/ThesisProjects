...
function ntptActivityTracking(evt){
  ...
  if( (evt !== null && typeof evt !== undefined) || 
    ...
    if( img !== null && typeof img.id !== "undefined" ){
      ...
      // Save to Project Tracking
       if( img.id.match(/save-to-project-*/)){
 	      var articles = "";     
	 	     if( img.id.match(/save-to-project-[a-zA-Z]*[0-9]+/)){
	 	    	 articles = img.id.replace('save-to-project-','');
	 	     } 
	 	     else if( articles == "" ){
		    	 articles =  ntptParseCitationArticleNumber("abstract");
		     } else {
		    	 articles = ntptParseCitationArticleNumber("contents");
		     }
	 	     if( articles !== "" ){
	 	        ntptCallEvent("ev=SaveToProject&file="+articles);
	 	     }
 	    }
     }
