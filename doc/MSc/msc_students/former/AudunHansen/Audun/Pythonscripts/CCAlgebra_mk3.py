...
class schematic():
    ...
    def find_permutations(self):
        ...
        for i in range(len(self.schema)):
            for e in range(len(self.schema[i])):
                if self.schema[i][e] == None:
                    orig.append([self.T_labels[i][e], self.T[i].q_c[e], "t"+str(e)])
        for i in range(len(self.H.q_c)):
            ...
            if contr.E() == E:
                for e in range(contr.Ndiag):
                    if config[0] == 1:
                        ...
                        print tx
                    if config[1] == 1:
                        print contr.diagram(e, [4*h,shift], True)
                    if config[1] == 2:
                        print contr.diagram(e, [4*h,shift], False)
                    if config[3] == 1:
                        contr.code(e)
                    shift += 4
    if config[1] == 2:
...
def combine_all(H,T, config = [1,0,0]):
    #Find combinations of operator elements in H, T that produce excitation level i
...
T = [["a"],["b"]]
print "T", T
print "TT", list_mult(T,T)
T2 = list_mult(T,T)
print "TTT", list_mult(T2,T)
print "TTT", list_pow(T,4)    
       
             
"""

H = Operator([1,1],[1,1])
#T = [Operator([],[1,-1]),Operator([],[1,1,-1,-1])]
#T = [[Operator([],[1,-1])],[Operator([],[1,1,-1,-1])]] #CCSD
T = [[Operator([],[1,-1])],[Operator([],[1,1,-1,-1])]] #CCD
