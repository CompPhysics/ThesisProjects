...
"""
The Operator object may be combined with other Operators to form a new Operator with a contracted core. 

The operation H*T may result in any number of new operators given by the stucture of H and T.

The operator object has a number of outgoing and incoming particle and hole lines, as well as a contracted core.

There are three options for interpreting the Operators:
    - Diagrammatic  (plots)
    - Mathematical  (latex)
    - Computational (C/C++)

When choosing the Computational interpretation, the operator returns a string for a C++ code to be implemented in a C++ solver.

Aim: To set up a 3-body Hamiltonian and produce a C++ CCSD(T) code. 
"""
...
class Operator():
    ...
    def assess_contributions(self, excitation_level = None):
        ...
        for i in self.I:
            ...
            (9)  Not yet implemented Permutations due to external lines 
            (10) Not yet implemented Cancel .5 factors 
            """
...
def qlist(o_cop):
    ...
    #search for pairs in O, create qvert-objects in a list, create operator
    
    
                                    
#T = cluster_operator([1,-1])
T = cluster_operator([1,1])
V9 = Operator([1,-1],[1,-1])
...
#generate_all_combinations(H,T, print_diag = 1)