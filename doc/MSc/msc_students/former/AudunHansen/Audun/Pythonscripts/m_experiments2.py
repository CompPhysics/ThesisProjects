...
class optiV():
    ...
    def sVhhpp(self):        
        ...
        for e in u:
            ...
            if len(Tp)!=0 and len(Th)!=0:
                ...
                data.shape = (len(Th), len(Tp))

                self.lVpphh.append([data, Th, Tp]) #store array and corresponding rows in dot product (dot(Vpphh, M)) #If transpose of data is needed in dot products, so is Th and Tp
        return self.lVpphh
    ...
    def sVhhhh2(self):        
        ...
        return coo_matrix((data, (T0, T1)), shape=(Nh**2, Nh**2)).toarray()
    def sVpphh2(self):        
        Np = self.Np
        Nh = self.Nh
        Nm = self.Nm
    ...
    def sVphhp22(self):
        ...
        idents_hp = bs.states[A,1:]+bs.states[I,1:]
        idents_hp = sum(array([1,Nm, Nm2, Nm*Nm2]) * idents_hp, 1)
        idents_ph = bs.states[J,1:]+bs.states[B,1:]
        idents_ph = sum(array([1,Nm, Nm2, Nm*Nm2]) * idents_ph, 1)
...
def composite_dot(M1,M2):
    #Z = dot(M1,M2), where M1 is a list of submatrices and corresponding rows in M2
...
#M = setup_Tpphh(bs)
T2 = flexitude(Np, Nh)
T2.M = random.randint(0,10,(Nh**2, Nh**2))
ret = op.sVpppp()
ret = op.sVhhhh()
#ret = op.sVpphh2() #works
ret = op.sVhpph22()
#imshow(ret)
#show()
#print ret
