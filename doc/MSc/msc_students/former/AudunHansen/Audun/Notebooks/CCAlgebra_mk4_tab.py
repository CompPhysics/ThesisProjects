...
class Combiner():
    ...
    def combine(self, H, ops, excitation = None):
        #Associate a list of T-operators (ops) to the current operator instance    
...
class symbolic_table():
    ...
    def build(self, target):
        ...
        if pre_fact==pre_denom:
            ...
            if pre_fact <0:
                s += "$-\\frac{%i}{%i}$" % (abs(pre_fact), pre_denom)
            else:
                s += "$\\frac{%i}{%i}$" % (abs(pre_fact), pre_denom)
        ...
        if len(interaction[0]) == 1:
            s += " f_{%s, %s}" % (interaction[0],interaction[1])
        else:
            s += " \\langle %s \\vert \\vert %s \\rangle" % (interaction[0],interaction[1])
        for i in cluster:
            s += " t_{%s}^{%s}" % (i[2], i[1])
        s += "$ " #\\" #+ "\\"
        #s += "\\"
...
class feynman_save():
    #a diagrammatic representation of the operators
    def __init__(self, name, schema, target, pos, vis = True):
        self.name = name
        self.schema = schema
        self.target = target
        self.pos = pos
...
class O():
    def __init__(self, H, T, name = "CC"):
        ...
        for e in range(len(self.schematics)):
            ...
            tab = symbolic_table(c,"%s" % (self.name+subindex[e]))
            self.stringforms.append(sym.stringform)
            self.tables.append(tab.stringform)
    ...
    def diagram(self,i, pos = [0,0], vis = True):
        try:
            ...
            print "Invalid index given:", i
            print "There are only %i diagrams in this contraction." % self.N
    def diagram_save(self,name, i, pos = [0,0], vis = True):

        try:
            dia = feynman_save(name, self.schematics[i], name, pos, vis)
        except:
            print "Invalid index given:", i
            print "There are only %i diagrams in this contraction." % self.N
...
def combine_to_excitation(H,T,E, config = [1,0,0,0], name = ""):
    ...
    for h in range(len(H)):
        ...
        for i in range(len(T)):
            ...
            if target_e == E:
                #print h,i
                t0 = 0
                stringname = name + "_%i_%i_%i" % (h,i, t0)
                ...
                if contr.E() == E:
                    for e in range(contr.Ndiag):
                        ...
                        if config[0] == 1:
                            ...
                            tex.append(tx)
                        if config[0] == 2:
                            tx = contr.latex_table(e) + " & \\includegraphics[height=10mm]{%s.png} \\\\" % stringname
                            contr.diagram_save(stringname, e, [4*h,shift], True)
                            tex.append(tx)
                        if config[1] == 1:
