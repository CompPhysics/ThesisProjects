...
class HEG:	#Homogenous Electron Gas
				...
				for ny in range(-n2,n2+1):
					for nz in range(-n2,n2+1):
						if (nx**2 + ny**2 + nz**2 == n2):
							...
							states.append(np.asarray([n2,nx,ny,nz,-1]))
							specials.append(np.asarray([self.]))
		NS = len(states)
		below_fermi = range(0, N)
		above_fermi = range(N, NS)
		return np.asarray(states), below_fermi, above_fermi, NS
	...
	def makeBlockMat(self):
		returnMat = []
		for kp in states:
			for kq in states:
				for kr in states:
					for ks in states:
						if np.prod(kp[1:-1]+kq[1:-1]==kr[1:-1]+ks[1:-1]):
							kuni = self.kUnique(kp[1:-1]+kq[1:-1])
							block = np.searchsorted(self.specials, kuni)
							returnMat.append(np.asarray([,,,]))
...
def D5():	#L_c
	factor 		= -1
	Val 		=  np.einsum('kajc,ikcb->ijab', Vhphp, T)	#can't change ikcb to cbik without using Vphph rather than Vhphp
...
def D6():	#Q_a
	factor 		= 0.25
	intMed		= np.einsum('ijcd,klcd->ijkl',T, Vhhpp)
	returnVal	= np.einsum('ijkl,klab->ijab',intMed, T)
	#returnVal 	= np.einsum('ijcd,klcd,klab->ijab', T, Vhhpp, T)
	return factor*returnVal
	
def D7():	#Q_b
	factor 		= 0.5
	intMed 		= np.einsum('ikca,klcd->iald', T, Vhhpp)
	Val			= np.einsum('iald,jldb->ijab', intMed, T)
...
def D8():	#Q_c
	factor 		= -0.5
	intMed 		=  np.einsum('klca,klcd->ad', T, Vhhpp)
	Val 		= np.einsum('ad,ijdb->ijab', intMed, T)
...
def D9():	#Q_d
	factor 		= -0.5
	intMed		=  np.einsum('ikcd,klcd->il', T, Vhhpp)
	Val 		=  np.einsum('il,jlab->ijab', intMed, T)
	returnVal 	=  Val - np.einsum('ijab->jiab', Val)
	return factor*returnVal
...
while conFac > eps:
	...
	#			for b in range(N,NS):
	#				if abs(F[j] + F[i] - F[a] - F[b]) > eps:
	#					T_new[i,j,a-N,b-N] = (Vhhpp[i,j,a-N,b-N] + D[i,j,a-N,b-N])/(F[j] + F[i] - F[a] - F[b])
