...
def temp2(inMat1,inMat2):	#i suggest inMat1 contains rows while inMat2 contains columns, convention is VERY important
	...
	for row in range(0,dim1):
		for col in range(0,dim2):
			vMat[row,col] = system.assym(inMat1[row,0],inMat1[row,1],inMat2[col,0],inMat2[col,1])
			if inMat1[row,0] == 4 and inMat1[row,1] == 10:
				print inMat1[row,0],inMat1[row,1],inMat2[col,0],inMat2[col,1]
	return vMat
...
def makeIntMatrix():	#just fyi, this function took me half a day, I'm not fluent in numpy functions
	blockArrays_pp_temp = np.zeros(((NS-N)**2,3))						#i really need to start using Np = NS-N
	blockArrays_hh_temp = np.zeros((N**2,3))
	...
	lastIndices_pp = np.zeros(distinct_ku_pp)
	lastIndices_hh = np.zeros(distinct_ku_hh)
	...
	for i in lastIndices_pp:
		blockArraysNew_pp.append(np.zeros((i+1-prev_i,3)))
...
print NS
Vpppp, idpppp, Vhhpp, idhhpp, F, idF, tempRet = makeIntMatrix()
