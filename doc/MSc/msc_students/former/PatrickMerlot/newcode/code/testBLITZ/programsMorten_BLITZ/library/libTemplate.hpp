...
#include "/site/Blitz++-0.8/include/blitz/array.h"
...
template <typename T> void choldc(Array<T,2>& A, Array<T,1>& P);
      /*
      ** construct a Cholesky decomposition of A assuming
      ** the Blitz matrix Array<T,2> to be positive definite 
      ** and symmetrix. On input, only the upper triangle needs
      ** to be given; it is not modified. The Cholesky factor
      ** L is returned in the lower part of A except for the 
      ** diagonal elements which are returned in Array<T,1> P.
      */
template<typename T> void cholsl(Array<T,2>& A, Array<T,1>& P, Array<T,1>& B, Array<T,1>& X);
      /*
      ** solves the set of n linear equation
      **         A * X = B
      ** where the Blitz matrix Array<T,2> A is a 
      ** positive-definite symmetrix matrix and 
      ** Array<T,1> P is output from the function 
      ** choldc(). Only the lower subdiaginal portion
      ** of A is accessed. The Array<T,1> B is the 
      ** right-hand input of the linear equations.
      ** The solution vector is returned in 
      ** Array<T,1> X. A and P are not modified and 
      ** can be left in place for successive calls
      ** with different right-hand sides B. B is not 
      ** modifided.
      */
...
void lubksb(Array<T,2>& A, int n, Array<int,1>& Index, Array<T,1>&  B)
{
   ...
   for(i = n - 1; i >= 0; i--) {
      ...
      */
template <typename T>
void choldc(Array<T,2>& A, Array<T,1>& P)
{
  int   i, j, k;
  T     sum;
  ...
  for(i = 0; i < A.rows; i++) {
	...
	if(sum <= 0.0) {
	  cout <<endl << endl 
	       <<"Error in function cholesky()"
	       <<endl <<"cholesky failed" << endl;
	  exit(1);
	}
	P(i) = sqrt(sum);
      }
...
template<typename T>
void cholsl(Array<T,2>& A, Array<T,1>& P, Array<T,1>& B, Array<T,1>& X)
{
  int     i, k;
  T       sum;
