...
!  block which contains interpolation functions
...
SUBROUTINE lagrange_2dim(x,y,a,fxy,dim,mesh_p)
  USE wave_functions
  USE constants
  IMPLICIT NONE
  ...
  REAL(DP), INTENT(IN) :: mesh_p(dim)
  REAL(DP), INTENT(IN) :: x, y
  REAL(DP), DIMENSION(dim) :: q
  REAL(DP) :: fxk, p
  fxy=0.d0
  nx0=dim
  ny0=dim
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!            Routines to do complex matrix inversion, from Numerical
...
SUBROUTINE cmplxlu_decompose(a,n,indx,d)
  ...
  complex(dpc) :: sum, dum, tiny, d, aamax
  complex(dpc), DIMENSION(n,n) :: a
  INTEGER, DIMENSION(n) :: indx
  complex(dpc), ALLOCATABLE :: vv(:)
...
!            | 1   0 |      | A^-1 0 |
! 
! the sign of the matrix AA is calculated using Denman-Beavers iteration method,
! see Higham et. al. ref. Numerical Algorithms 15 (1997) 227-242 
! 
subroutine sqrtmat_db( aa, a, a_inv ,n ) 

  implicit none
  integer, intent(in) :: n
  complex*16, dimension(n,n), intent(in) :: aa
  complex*16, dimension(n,n), intent(out) :: a, a_inv
  complex*16, dimension(n,n) :: z,y, z0, y0, z1, y1, z1_inv, y1_inv
  ...
  i_mat = 0.
  do i = 1, n
     i_mat(i,i) = dcmplx(1.d0,0.d0)
  end do
  !setup starting matrix z0, y0 
  z0 = i_mat
  y0 = aa
  ...
  k = 0
  temp = 0.
  z1 = 0.; y1 = 0.;z1_inv = 0.;y1_inv = 0.;y = 0.; z = 0.;
  do while( maxval(abs(temp-aa)) > 1.e-8 .and.  k < 1000 )
     
     z1 = z0 
     y1 = y0 
     z1_inv = z0
     y1_inv = y0 
     call cmplxmatinv(z1_inv,n,d)
     call cmplxmatinv(y1_inv,n,d)
...
!  write(6,*) 'total number of iterations in sqrt_mat:', k
  
  
end subroutine sqrtmat_db
...
! http://www.csit.fsu.edu/~burkardt/f_src/quadrule/quadrule.html
...
!
!    Input, real ( kind = 8 ) WEIGHT(NORDER), the weights of the quadrature rule.
!
!    Output, real ( kind = 8 ) RESULT, the approximate value of the integral.
!
  implicit none
