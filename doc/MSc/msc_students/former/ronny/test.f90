...
!****************************************************************************************************
! This subroutine converts the densities to find the fermilevels.
!
...
!****************************************************************************************************
! Now we must read from	the chem.dat file to get the value for the electron/muon we need in the	calculations
! to find the emissivity.
...
! *10.0d0**(-27.0d0)*10.0d0**(-27.0d0)=10.0d0**(-54.0d0)*(457pi/10080)(kBT)**6)
!
!
...
!		Set up mesh points and weights for all particles..
		...
		CALL gauleg(0.0d0,  300.0d0, mp_neutron_y,		wgt_neutron_y,		nump)
		CALL gauleg(0.0d0,  300.0d0, mp_proton_y,		wgt_proton_y,		nump)
		CALL gauleg(0.0d0,  300.0d0, mp_electron_y,	    wgt_electron_y,		nump)
		CALL gauleg(0.0d0,  300.0d0, mp_muon_y,		    wgt_muon_y,			nump)
		CALL gauleg(0.0d0,  300.0d0, mp_sigma_minus_y,	wgt_sigma_minus_y,	nump)
		CALL gauleg(0.0d0,  300.0d0, mp_lambda_y,		wgt_lambda_y,		nump)
		CALL gauleg(0.0d0,  300.0d0, mp_sigma0_y,		wgt_sigma0_y,		nump)
		CALL gauleg(0.0d0,  300.0d0, mp_sigma_pluss_y,	wgt_sigma_pluss_y,	nump)
...
! Then the process  lambda > p e v
	   ...
	   .AND.  fermilevels_array%electron(i)	+ fermilevels_array%proton(i) -	&
	       fermilevels_array%lambda(i) > 0)	THEN
	epsilon_array%epsilon_nu_lpe3(i)=0
				DO ix=1,nump
					DO iy=1,nump
