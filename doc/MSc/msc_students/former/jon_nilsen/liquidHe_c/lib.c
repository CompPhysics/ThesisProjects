...
void  *mem_calloc(size_t num_elements, size_t size, char *funct, char *var)
...
double trapezoidal_rule ( double, double, int, double (*)(double) );
    ...
    **     These function evaluates an integral using the trapezoidal rule 
    **     giving as input the integration limits, mesh points and a pointer 
    **     to the name of the  function to integrate. It returns the results 
    **     as a double value                   
      
double simpson ( double, double, int, double (*)(double) );

    **     These function evaluates an integral using simpson's rule 
    **     giving as input the integration limits, mesh points and a pointer 
    **     to the name of the  function to integrate. It returns the results 
    **     as a double value                                      
...
#undef     calloc   
   ...
   if((new_memory = (void *) calloc(num_elem, size)) == NULL)  {
      ...
      printf("\nmemory allocation from function %s: %s ", func, var);
      printf(" \nMemory request: number of elements = %d  size of elements = %d bytes \n",
                                                                            num_elem, size);
      exit(1);
   }
...
#undef     malloc
    ...
    pointer = (void  **) malloc((UL)( row * sizeof(void  *)));
    if(!pointer )  {
    ...
    for( loop = 0; loop < row; loop++)  {
      pointer[loop] = (void  *) malloc((UL)(col * num_bytes));
      if( !pointer[loop] )  {
        printf("\nNo enough memory to store a matrix with");
        printf(" \ncol = %d\n",col);
        exit(1);
	                ...
	                void (*derivs)(double, double *, double *))
{
   ...
   dym = MALLOC(n, double, func, "dym[]");
   dyt = MALLOC(n, double, func, "dyt[]");
   yt  = MALLOC(n, double, func, "yt[]");
...
void ludcmp(double **a, int n, int *indx, double *d)
{
   ...
   vv = MALLOC(n, double, func, "vv[]");  /* memory allocation */
...
void polint(double xa[], double ya[], int n, double x, double *y, double *dy)
{
   ...
   c = MALLOC(n, double, func, "c[]");
   d = MALLOC(n, double, func, "d[]");
