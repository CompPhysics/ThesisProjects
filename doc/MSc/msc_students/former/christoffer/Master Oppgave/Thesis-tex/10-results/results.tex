...
We test our program by running with one filled shell (two electrons) using an effective interaction, and observe how we get the exact value of $3[Ha]$ for $\omega=1$ irrespective of the basis size.
...
Summarizing, we reproduce earlier results, the code is consistent with full diagonalization for $2$ particles, and effective interactions are used correctly.
...
The different implementations for matrix multiplication are first tested by calculating the matrix product of two matrices, $A$ and $B$, defined in~\eqref{eq:results:ABsize}, filled with random numbers.
...
\begin{center}
\subfigure[$N=2$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultCC_1.pdf}
}
\subfigure[$N=12$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultCC_3.pdf}
}
\subfigure[$N=30$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultCC_5.pdf}
}
\subfigure[$N=56$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultCC_7.pdf}
}
...
\begin{center}
\subfigure[$N=2$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultHFsys_1.pdf}
}
\subfigure[$N=12$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultHFsys_3.pdf}
}
\subfigure[$N=30$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultHFsys_5.pdf}
}
\subfigure[$N=56$]{
\includegraphics[scale=0.85]{../10-results/figs/timeMult/timeMultHFsys_7.pdf}
}
...
The largest test takes 160 minutes, where almost 100 of them are spent calculating the transformed HF basis.
Matrix multiplication stands for more than $60$\% of wall time.
We see that further optimizations should be targeted at the basis transformation first, hereunder find a more efficient way of multiplying the large square matrices.
\begin{table}
\begin{center}
\caption{Time used for different parts of the calculations. Run on a fat node with 128GB of memory. Matrix-multiplications are accelerated by the use of a GPU in the `CLstrassen' class. As GotoBLAS was not successfully installed here, the GPU implementation is used.}
...
\end{center}
\end{table}
For the coupled-cluster part, which now takes up only a fourth of all execution time, there is evidence of a bottleneck other than matrix multiplication.
...
\hline
Init. maps & 19.9 \\
$\left[\mathcal{I}_{11}\right]$  \#2 & 17.5 \\
$ \hat{T}_2$  \#2 & 9.4 \\
$ \hat{T}_2$  \#8 & 4.8 \\
$\left[\mathcal{I}_{10}\right]$ &  3.7 \\
\hline \hline
\end{tabular}
}
\hspace{5mm}
...
Part & Time [s] \\
\hline
$\left[\mathcal{I}_{11}\right]$  \#2 & 656 \\
Init. maps & 455 \\
$ \hat{T}_2$  \#2 & 329 \\
$\left[\mathcal{I}_{10}\right]$ & 152 \\
$ \hat{T}_2$  \#8 & 140 \\
\hline \hline
\end{tabular}
}
...
When the frequency drops to $\omega = 0.1$, as in fig.~\ref{fig:results:convN2OM01}, we still see a fully converged Hartree-Fock energy for the standard interaction, and a slightly slower convergence for the effective.
...
The last case we study is $N=20$, $30$ and $42$ with a confinement strength of $\omega = 0.7$.
...
Also studied are correlations with respect to the Hartree-Fock energy, defined similar to $\chi$, except replacing $\langle \hat{H}_0 \rangle$ with $E_{HF}$,
\begin{equation}
...
\begin{figure}
\begin{center}
\subfigure[$N=2,6$ and $12$]{
\includegraphics[scale=1.35]{../10-results/figs/loweringOmega/correlationMeanField_2_6_12_Zoom.pdf}
}
\subfigure[$N=20,30,42$ and $56$]{
\includegraphics[scale=1.4]{../10-results/figs/loweringOmega/correlationMeanField_20_30_42_56_Zoom.pdf}
}
...
Actual results are found in table~\ref{tab:results:otherMethods}.
...
\begin{table}
\begin{center}
\caption{Comparison with full configuration-interaction (FCI) results supplied by fellow master's student F. B. Olsen. The differing decimals are highlighted.}
\subfigure[$N=6$]{
\label{tab:results:otherFCI}
\begin{tabular}{rr|ccc}
$\omega$ & $R$ & CCSD & FCI & $\Delta$ \\
\hline \hline 
    &  3 & 21.42\textbf{323172} & 21.42\textbf{062118} & $3 \cdot 10^{-3}$ \\
    &  4 & 20.4\textbf{2820553} & 20.4\textbf{1589181} & $1 \cdot 10^{-2}$ \\
1.0 &  5 & 20.3\textbf{3138914} & 20.3\textbf{1679827} & $1 \cdot 10^{-2}$ \\
    &  6 & 20.2\textbf{7324671} & 20.2\textbf{5725213} & $2 \cdot 10^{-2}$ \\
    & 30 & 20.18349343 &  & $9 \cdot 10^{-2}$ \\
 & & \\
    &  3 & 12.\textbf{90122455} & 12.\textbf{89723259} & $4 \cdot 10^{-3}$ \\
    &  4 & 12.0\textbf{5615503} & 12.0\textbf{3695836} & $2 \cdot 10^{-2}$ \\
0.5 &  5 & 11.9\textbf{3410621} & 11.9\textbf{1311827} & $2 \cdot 10^{-2}$ \\
    &  6 & 11.8\textbf{6345461} & 11.8\textbf{4042104} & $2 \cdot 10^{-2}$ \\
    & 30 & 11.81170403 &  & $5 \cdot 10^{-2}$ \\
 & & \\
 	&  3 & 4.\textbf{208076509} & 4.\textbf{149563782} & $6 \cdot 10^{-2}$ \\
 	&  4 & 3.\textbf{829020715} & 3.\textbf{797449624} & $3 \cdot 10^{-2}$ \\
0.1 &  5 & 3.6\textbf{66397817} & 3.6\textbf{48168769} & $2 \cdot 10^{-2}$ \\
	&  6 & 3.5\textbf{96913442} & 3.5\textbf{68045420} & $3 \cdot 10^{-2}$ \\
...
$\omega$ & $R$ & CCSD & FCI & $\Delta$ \\
\hline \hline 
1.0	&  4 & 70.3\textbf{2360770} & 70.3\textbf{1288325} & $1 \cdot 10^{-2}$ \\
