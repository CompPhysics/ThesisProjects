...
%%% added by Simen Kvaal:
Amults = 0;
global phi2;  %% analytic solution computed elsewhere...
my_error = [];
%%%
...
while t_now < t_out
  ...
  for j = 1:m
     p = A*V(:,j);
     Amults = Amults + 1; % line added by Simen Kvaal.
     for i = 1:j
        H(i,j) = V(:,i)'*p;
        p = p-H(i,j)*V(:,i);
     end;
...
%% added by Simen Kvaal, june 2004:
fprintf('total number of steps: %d\n', nstep);
fprintf('total number of A-mults: %d\n', Amults);
